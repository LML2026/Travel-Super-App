...
class LocalIntentClassifier implements IntentClassifier {
  ...
  AssistantIntent classify(String message) {
    ...
    if (_containsAny(
        normalized, ['open my profile', 'show profile', 'my profile'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(
        normalized, ['plan a new trip', 'plan my trip', 'create a trip'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(normalized,
        ['open my trips', 'show trips', 'trip list', 'show my trip'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(normalized,
        ['find places', 'show places', 'places near me', 'discover places'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(normalized,
        ['recommend an activity', 'find an activity', 'what should i do'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(
        normalized, ['find a pharmacy', 'nearby pharmacy', 'pharmacy'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(normalized,
        ['find a hospital', 'nearby hospital', 'hospital near me'])) {
      return const AssistantIntent(
    ...
    if (_containsAny(normalized,
        ['add this place to my itinerary', 'add place to itinerary'])) {
      return const AssistantIntent(
